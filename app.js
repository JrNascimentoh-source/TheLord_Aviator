...
function renderMonitor(){
  ...
  const factEl=document.getElementById('monitorFact');
  const grid=document.getElementById('roundGrid');
  if(!grid)return;

  if(!rounds.length){
    factEl.innerHTML='Registre rodadas para acompanhar o histórico real do jogo.';
    grid.innerHTML='<div class="round-empty">Nenhuma rodada registrada ainda. Use o campo acima para adicionar.</div>';
  }else{
    const twentyMinAgo=Date.now()-20*60000;
    const highLast20=rounds.filter(r=>(r.value!==null && r.value>=10 || r.color==='pink' || r.color==='hot') && r.ts>=twentyMinAgo).length;
    factEl.innerHTML=`<b>${highLast20}</b> vela${highLast20===1?'':'s'} ≥10x nos últimos 20 minutos, de um total de <b>${rounds.length}</b> rodadas registradas.`;
    grid.innerHTML=rounds.slice(0,60).map((r,i)=>{
      ...
      return `<div class="round-chip chip-${cls}"><span class="round-chip-idx">${idx}</span><div class="round-chip-val">${valLabel}</div><div class="round-chip-time">${nowTimeStr(new Date(r.ts))}</div></div>`;
    }).join('');
  }

  const withValue=rounds.filter(r=>r.value!==null && r.value!==undefined);
  const lastHigh=withValue.find(r=>r.value>=10);
  document.getElementById('statLastHigh').textContent=lastHigh?lastHigh.value.toFixed(2)+'x':'—';
  document.getElementById('statLastHighTime').textContent=lastHigh?nowTimeStr(new Date(lastHigh.ts))+' · '+timeAgo(lastHigh.ts):'Nenhuma ainda';

  const dayAgo=Date.now()-24*3600000;
  const lastExtreme=withValue.find(r=>r.value>=100 && r.ts>=dayAgo);
  document.getElementById('statLastExtreme').textContent=lastExtreme?lastExtreme.value.toFixed(2)+'x':'—';
  document.getElementById('statLastExtremeTime').textContent=lastExtreme?nowTimeStr(new Date(lastExtreme.ts))+' · '+timeAgo(lastExtreme.ts):'Nenhuma ainda';

  const last1050=withValue.find(r=>r.value>=10 && r.value<50);
  document.getElementById('stat1050').textContent=last1050?last1050.value.toFixed(2)+'x':'—';
  document.getElementById('stat1050Time').textContent=last1050?timeAgo(last1050.ts):'Nenhuma ainda';

  const last50100=withValue.find(r=>r.value>=50 && r.value<100);
  document.getElementById('stat50100').textContent=last50100?last50100.value.toFixed(2)+'x':'—';
  document.getElementById('stat50100Time').textContent=last50100?timeAgo(last50100.ts):'Nenhuma ainda';

  const hourAgo=Date.now()-3600000;
  const lastHour=rounds.filter(r=>r.ts>=hourAgo);
  const counts={blue:0,purple:0,pink:0,hot:0};
  lastHour.forEach(r=>counts[r.color||classifyRound(r.value).key]++);
